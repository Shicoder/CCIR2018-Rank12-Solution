...
class GRU4Rec:
    ...
    def generate_neg_samples(self, pop, length):
        ...
        return sample
    def fit(self, data, valid, retrain=False, sample_store=10000000):
        '''
        Trains the network.
        ...
        if not retrain:
            ...
            if n_new_items:
                ...
                self.logger.info('Added {} new items. Number of items is {}.'.format(n_new_items, self.n_items))
            data = pd.merge(data, pd.DataFrame({self.item_key:itemids, 'ItemIdx':self.itemidmap[itemids].values}), on=self.item_key, how='inner')
            data.sort_values([self.session_key, self.time_key], inplace=True)
            offset_sessions = np.zeros(data[self.session_key].nunique()+1, dtype=np.int32)
            offset_sessions[1:] = data.groupby(self.session_key).size().cumsum()
        ...
        if self.n_sample:
            ...
            if sample_store:
                ...
                if generate_length <= 1:
                    sample_store = 0
                    self.logger.info('No example store was used')
                else:
                    neg_samples = self.generate_neg_samples(pop, generate_length)
                    sample_pointer = 0
                    self.logger.info('Created sample store with {} batches of samples'.format(generate_length))
            else:
                self.logger.info('No example store was used')
        ...
        for epoch in range(self.n_epochs):
            ...
            avgc = np.mean(c)
            if np.isnan(avgc):
                self.logger.error('Epoch {}: NaN error!'.format(str(epoch)))
                self.error_during_train = True
                return
            if epoch % 5 == 0:
              res = self.evaluate_sessions_batch(valid, None, batch_size=self.batch_size)
              self.logger.info('Epoch: {} loss: {:.6f} Recall@20: {} MRR@20: {}'.format(epoch, avgc, res[0], res[1]))
        ...
        while True:
            ...
            for i in range(minlen-1):
                ...
                if sampled_items:
                    ...
                    # 这句出错了
                    preds = self.predict_next_batch(iters, in_idx, None, batch_size) #TODO: Handling sampling?
